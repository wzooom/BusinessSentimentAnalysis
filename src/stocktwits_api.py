...
from src.api_access.config import RAW_DATA_DIR, ensure_data_dirs, get_env
...
def fetch_stocktwits_messages(
    ...
    """
    Pull as many StockTwits messages as StockTwits exposes through pagination.

    Saves progress after every page. If you stop with Control+C, rows stay saved.
    Rerunning resumes from the existing raw CSV and skips duplicate message IDs.
    """
    ensure_data_dirs()
    ...
    # Resume older pagination from the smallest existing message_id if available.
    if not existing.empty and "message_id" in existing.columns:
